...
if [ -z ${AWS_BATCH_JOB_MAIN_NODE_PRIVATE_IPV4_ADDRESS+x} ]; then
    ...
    $DIR/build/paracs --cadical-cubes  --cadical-cubes-depth 17 --cadical-minimal-cubes-depth 12 --resplit $DIR/build/problem.cnf --worker $DAEMON_THREADS $@
else
    $DIR/build/paracs --worker $DAEMON_THREADS --known-remote ${AWS_BATCH_JOB_MAIN_NODE_PRIVATE_IPV4_ADDRESS} $Q
fi
