...
if [ -z ${AWS_BATCH_JOB_MAIN_NODE_PRIVATE_IPV4_ADDRESS+x} ]; then
    ...
    $DIR/build/parac --cadical-cubes  --cadical-cubes-depth 17 --cadical-minimal-cubes-depth 12 --resplit-cubes $DIR/build/problem.cnf $@
else
    $DIR/build/parac --resplit-cubes  --daemon --threads $DAEMON_THREADS $Q
fi
