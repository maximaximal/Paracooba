...
if [ "$AWS_BATCH_JOB_MAIN_NODE_INDEX" = "$AWS_BATCH_JOB_NODE_INDEX" ]; then
    ...
    $DIR/build/paracs --cadical-cubes --initial-cube-depth 15 --initial-minimal-cube-depth 12 --resplit $DIR/build/problem.cnf --concurrent-cube-tree-count 4 --worker $NUM_PROCESSES --id $id -d --tcp-listen-address 0.0.0.0 -t $@
else
    ...
    $DIR/build/paracs --worker $NUM_PROCESSES --known-remote ${AWS_BATCH_JOB_MAIN_NODE_PRIVATE_IPV4_ADDRESS} --auto-shutdown-after-finished-client --tcp-listen-address 0.0.0.0 --id $id -d -t $Q
fi
