  ...
  , m_optionsFile("Config file options")
{
  ...
  m_optionsCommon.add_options()
    ...
    ("disable-autodiscovery", po::bool_switch(&m_disableAutoDiscovery)->default_value(false)->value_name("bool"), "enable auto discovery via UDP broadcasts. Gets deactivated if --known-remotes is used or if nodes are added via envvars.")
    ("disable-tcp-auto-port-assignment", po::bool_switch(&m_disableTCPAutoPortAssignment)->default_value(false)->value_name("bool"), "enable automatic port assignment for TCP listen port. Can be handy if many nodes are started on same host. Tries binds until it succeeds (or reaches limit of 65535).")
