...
if [ -z ${AWS_BATCH_JOB_MAIN_NODE_PRIVATE_IPV4_ADDRESS+x} ]; then
    ...
    ./scripts/generate-cubes.sh $DIR/build/problem.cnf > $DIR/build/problem.icnf
    $DIR/build/parac $DIR/build/problem.icnf $@
else
    $DIR/build/parac --daemon --threads $DAEMON_THREADS $Q
fi
